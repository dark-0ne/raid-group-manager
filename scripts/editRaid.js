...
function showCharacterInRaid(character, role) {
    const row = document.createElement("div")
    row.classList.add("row", "align-items-center", "py-1", "my-1", "character-row")
    row.setAttribute("name", character.character_name)
    ...
    switch (role) {
        case "tank":
            document.getElementById("tank-container").appendChild(row)
            raidTanks.push(character)
            break

        case "healer":
            document.getElementById("healer-container").appendChild(row)
            raidHealers.push(character)
            break

        case "dps":
            document.getElementById("dps-container").appendChild(row)
            raidDps.push(character)
            break
    }
    ...
    client.connect(async (err) => {
        if (err) {
            ...
            try {
                ...
                setTimeout(() => {
                    window.location.replace("raids.html");
                }, 1000);
...
const raidID = location.search.substring(3)
    ...
    if (err) {
        ...
        raid_col.findOne({
            ...
            if (err) {
                ...
                fixedTime.setTime(raid.date.getTime()-raid.date.getTimezoneOffset()*60000)
                fixedTime = fixedTime.toISOString()
                document.getElementById("raid-title").value = raid.title
                document.getElementById("breadcrumb-id").textContent = raid.title
                document.getElementById("raid-date").value = fixedTime.substring(0,fixedTime.length-5)

                switch (raid.size) {
                    case "10-man":
                    case "25-man":
                        document.getElementById("raid-size").value = raid.size
                        break
                    default:
                        document.getElementById("raid-size").value = "other"
                        document.getElementById("size-other").value = raid.size
                        document.getElementById("size-other-col").classList.remove("d-none")
                }
                ...
                switch (raid.instance) {
                    ...
                    case "Trial of the Crusader":
                        document.getElementById("raid-instance").value = raid.instance
                        break
                    default:
                        document.getElementById("raid-instance").value = "other"
                        document.getElementById("instance-other").value = raid.instance
                        document.getElementById("instance-other-col").classList.remove("d-none")
                }

                document.getElementById("raid-notes").value = raid.notes
                document.getElementById("dkpCheckbox").checked = raid.dkp

                for (character of raid.tanks) showCharacterInRaid(character, "tank")
                for (character of raid.healers) showCharacterInRaid(character, "healer")
                for (character of raid.dps) showCharacterInRaid(character, "dps")

                document.getElementById("loader").classList.add("d-none")
                document.getElementById("cover-container").classList.remove("d-none")
